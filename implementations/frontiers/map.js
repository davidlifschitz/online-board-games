...
const TERRITORY_BY_ID = Object.fromEntries(TERRITORIES.map(x => [x.id, x]));
const ADJACENCY = Object.fromEntries(TERRITORIES.map(x => [x.id, [...x.neighbors]]));
const REGION_TERRITORIES = Object.fromEntries(Object.keys(REGIONS).map(region => [region, TERRITORIES.filter(t => t.region === region).map(t => t.id)]));
